...
const upload = (token, hook, code, problem, filename, sha, commitMsg, cb = undefined) => {
  const URL = `https://api.github.com/repos/${hook}/LeetCode/${difficulty}/${problem}/${filename}`;
...
async function getUpdatedData(token, hook, directory, filename) {
  const URL = `https://api.github.com/repos/${hook}/LeetCode/${difficulty}/${directory}/${filename}`;
